...
*/
	...
	int psudoBurst[4] = {30, 50, 150, 80};
	for(int n=1; n<=4; n++){
		// simulate four threads
		t[n] = new Thread(psudoName[n-1]);
		t[n]->priorityLevel = psudoPriority[n-1];
		t[n]->burstTime = psudoBurst[n-1];

		t[n]->Fork((VoidFunctionPtr) &PsudoExecute, (void *)NULL);
